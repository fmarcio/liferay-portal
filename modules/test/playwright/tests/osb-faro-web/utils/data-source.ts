...
export async function renameDataSource(
	...
	await page
		.getByRole('link', {exact: true, name: oldDataSourceName})
		.click();
...
export async function searchDataSourceAndClickOnIt(
	dataSourceName: string,
	page: Page
) {
	...
	await page.getByRole('link', {exact: true, name: dataSourceName}).click();
}
